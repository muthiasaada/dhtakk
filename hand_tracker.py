...
class HandTracker(object):
    ...
    def get_contour(self, binaryIm, checkCentroid=True):
        ...
        binIm = 1*binaryIm
        hierarchy, contours, hierarchy = cv2.findContours(binIm,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
        maxArea = 0
        ci = -1
    ...
    def initialize_contour(self, binaryIm):
        #binaryIm = self.get_binary_image(imhsv)
        hierarchy, contours, hierarchy = cv2.findContours(binaryIm,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
        maxArea = 0
        ci = -1
        for i,c in enumerate(contours):
            h = cv2.convexHull(c)
